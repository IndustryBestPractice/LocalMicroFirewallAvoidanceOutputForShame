...
def index(request):
	if request.user.is_authenticated:
            if request.method == "POST":
                choice_var = request.POST.get('choice', False)
                ...
                src_query = Events.objects.values('src_ip_id').distinct().filter(date=str(latest_date),src_ip_id__in=IPAddress.objects.only('id').values_list('id', flat=True))
                src_query.count()
                # Get all data from Events where the dst_ip_id is in the IPAddress>
                dst_query = Events.objects.values('dst_ip_id').distinct().filter(date=str(latest_date),dst_ip_id__in=IPAddress.objects.only('id').values_list('id', flat=True))
                dst_query.count()
                ...
                edges = Events.objects.filter(date=str(latest_date)).values('src_ip_id','dst_ip_id','dstport','action','path').annotate(total=Count('id'))
                # COOKIES AND STUFF!
